// extensions/tier-pricing-discount/node_modules/@shopify/shopify_function/run.ts
...
// extensions/tier-pricing-discount/src/cart_lines_discounts_generate_run.js
function cartLinesDiscountsGenerateRun(input) {
  ...
  if (configType === "fixed_bundle" || configType === "multi_tier_volume" || configType === "cart_wide_volume" || configType === "mix_match_volume") {
    ...
    const productPrices = uniqueProductIds.map((productId) => {
      ...
      const maxPrice = Math.max(...lines.map((line2) => parseFloat(line2.cost.amountPerQuantity.amount)));
    ...
    if (totalDiscount > 0) {
      ...
      for (let i = 0; i < bundleProducts.length; i++) {
        ...
        candidates.push({
          ...
          message: config.message || config.title || "Volume Discount Applied \u{1F389}"
        });
...
// extensions/tier-pricing-discount/src/cart_delivery_options_discounts_generate_run.js
